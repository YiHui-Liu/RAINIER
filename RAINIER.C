...
// you'll have to write code to generate this or you can do it manually. I recompiled TALYS and wrote a parser to do
// this neatly.
...
void ReadDisInputFile() {
  ...
  while (!bFoundNuc && nTry < nTries) {
    ...
    if (nA == g_nAMass && nZ == g_nZ) {
      bFoundNuc = true;
      cout << "Nucleus: " << endl << sNucSearchLine << endl;
    }
  }
  if (nLvlTot < 2) {
    cerr << "err: No levels, check z file" << endl;
    cin.get();
  }
...
void ReadPopFile() {
  ...
  g_h2PopDist = new TH2D("h2PopDist", "h2PopDist", g_nSpPopIBin, 0, g_nSpPopIBin, g_nExPopI - 1, adEx);
...
double GetSpinCut2(double dEx) {
  ...
  double dSpinCut2 = 0.0146 * pow(g_nAMass, 5.0 / 3.0) * (1 + sqrt(1 + 4 * dLDa * dEff)) / (2 * dLDa);
#endif
  ...
  if (dExPd < 0)
    dExPd = 0.000001;
  double dSpinCut2 = 0.391 * pow(g_nAMass, 0.675) * pow(dExPd, 0.312);
#endif

#ifdef bJCut_TALYS // TALYS 1.8 default:
  double dSpinCutF2 = 0.01389 * pow(g_nAMass, 5.0 / 3.0) / g_dLDaAsym * sqrt(dLDa * dEff);

  double dEffSn = GetEff(g_dSn);
  double dLDaSn = GetLDa(g_dSn);
  double dSpinCutSn2 = 0.01389 * pow(g_nAMass, 5.0 / 3.0) / g_dLDaAsym * sqrt(dLDaSn * dEffSn);
  ...
  if (dEx <= g_dEd) {
    ...
    dSpinCut2 = dSpinCutd2 + (dEx - g_dEd) / (g_dSn - g_dEd) * (dSpinCutSn2 - dSpinCutd2);
  } else {
...
double GetDensityTot(double dEx) {
  ...
  double dEnDen =
      1.0 / (12.0 * sqrt(2) * sqrt(dSpinCut2) * pow(dLDa, 0.25) * pow(dEff, 5.0 / 4.0)) * exp(2.0 * sqrt(dLDa * dEff));
#endif
...
double GetInBinE(int nReal, int nConEx, int nSpb, int nPar, int nLvlInBin) {
  TRandom2 ranInBinE(1 + // have seen issues with 0 seed
                     nPar + nConEx * 2 + nReal * 2 * g_nConEBin + nSpb * 2 * g_nConEBin * g_nReal +
  ...
  for (int ex = 0; ex < g_nConEBin; ex++) {
    ...
    for (int spb = 0; spb < g_nConSpbMax; spb++) {
      for (int par = 0; par < 2; par++) {
        ...
        if (nRanNumLvl > g_nConMaxLvlBin)
          g_nConMaxLvlBin = nRanNumLvl;
    ...
    for (int spb = 0; spb < g_nConSpbMax; spb++) {
      ...
      for (int ex = 0; ex < g_nConEBin; ex++) {
        while (dWigSampleSum < adExpCumulCon[ex]) {
          dWigSampleSum += 2.0 / sqrt(g_dPi) * sqrt(-log(ranLvlGen.Uniform(1.0)));
...
int GetTransType(int nSpbI, int nParI, int nSpbF, int nParF) {
  ...
  if (nSpbF < 0 || nSpbF >= g_nConSpbMax)
    return 0; // not possible
  if (nSpbI < 0)
    cerr << "err: Negative input spins" << endl;
...
double GetStrE1(double dEx, double dEg) {
  ...
  for (int set = 0; set < g_nParE1; set++) { // sum over split dipoles in applicable
    ...
    double dTerm1 = dEg * dGam / (pow((dEg * dEg - g_adEneE1[set] * g_adEneE1[set]), 2) + pow(dEg * dGam, 2));
    double dTerm2 = 0.7 * g_adGamE1[set] * g_d4Pi2 * dTemp * dTemp / pow(g_adEneE1[set], 5);
    ...
    double dTerm = 0.7 * g_adEneE1[set] * dGam / pow(dEg * dEg - g_adEneE1[set] * g_adEneE1[set], 2);
#endif

#ifdef bE1_KopChr // Kopecky Chrien
    double dTerm = dEg * dGam / (pow((dEg * dEg - g_adEneE1[set] * g_adEneE1[set]), 2) + pow(dEg * dGam, 2));
#endif

#ifdef bE1_StdLor // Standard Lorentzian
    double dTerm =
        dEg * g_adGamE1[set] / (pow(dEg * dEg - g_adEneE1[set] * g_adEneE1[set], 2) + pow(dEg * g_adGamE1[set], 2));
#endif
...
double GetStrM1(double dEg) {
  ...
  for (int set = 0; set < g_nParM1; set++) {
    ...
    dStr += g_dKX1 * g_adSigM1[set] * dEg * g_adGamM1[set] * g_adGamM1[set] /
            (pow(dEg * dEg - g_adEneM1[set] * g_adEneM1[set], 2) + pow(dEg * g_adGamM1[set], 2));
#endif
...
double GetRanChi2(TRandom2 &ran, double dnu) {
  ...
  if (dk == 1) { // exponential dist
    ...
    while (!bAccept) {
      ...
      if (dx > 0) {
        ...
        if ((dv3 <= 1 - 2 * dv2 * dv2 / dx) || (log(dv3) <= 2 * ((dk - 1) * log(dx / (dk - 1)) - dv2))) {
          bAccept = true;
          dAcceptX = dx;
        } // v3 condtion
      } // x condtion
  ...
  if (dAcceptX < 0)
    cerr << "err: no ch2 random assigned" << endl;
  return dAcceptX / g_dlam;
} // GetRanChi2

double GetStr(double dEx, double dEg, int nTransType, double &dMixDelta2, TRandom2 &ranStr) {
  // returns sum_XL( Str_XL * Eg^(2L+1) )
  dMixDelta2 = 0.0;
  switch (nTransType) {
  case 0:
    return 0.0; // save some flops for impossible transistions
...
// int nX = 0, nL = 2;
  ...
  default:
    cerr << "err: Invaild strength type" << endl;
    return 0.0;
  } // TransType
} // GetStr
...
double GetBrICC(double dEg, int nTransMade = 1, double dMixDelta2 = 0.0) {
  ...
  case 0:
    ...
    break;
  case 1:
    nSuccess = system(Form("./%s -Z %d -g %f -L E1 -w %s > oAlpha.briccs", cbriccs, g_nZ, dEg * 1000, g_sBrIccModel));
    nReadLine = 8;
    break; // MeV
  case 2:
    nSuccess = system(Form("./%s -Z %d -g %f -L M1+E2 -d %f -w %s > oAlpha.briccs", cbriccs, g_nZ, dEg * 1000,
                           sqrt(dMixDelta2), g_sBrIccModel));
    nReadLine = 11;
    break;
  case 3:
    nSuccess = system(Form("./%s -Z %d -g %f -L M1 -w %s > oAlpha.briccs", cbriccs, g_nZ, dEg * 1000, g_sBrIccModel));
    nReadLine = 8;
    break;
  case 4:
    nSuccess = system(Form("./%s -Z %d -g %f -L E2 -w %s > oAlpha.briccs", cbriccs, g_nZ, dEg * 1000, g_sBrIccModel));
    nReadLine = 8;
    break;
  default:
    cerr << "err: impossible transistion" << endl;
  } // transistion
  if (nSuccess)
    cerr << "err: BrIcc failure" << endl;
  ifstream fileAlpha("oAlpha.briccs");
  string sAlphaLine;
  for (int line = 0; line < nReadLine; line++) {
...
double GetICC(double dEg, int nTransMade = 1, double dMixDelta2 = 0.0) {
  ...
  case 0:
    ...
    break;
  case 1:
    return g_hE1ICC->Interpolate(dEg);
    break;
  case 2: {
    double dM1 = g_hM1ICC->Interpolate(dEg);
    double dE2 = g_hE2ICC->Interpolate(dEg);
    return (dM1 + dMixDelta2 * dE2) / (1 + dMixDelta2);
  } break;
  case 3:
    return g_hM1ICC->Interpolate(dEg);
    break;
  case 4:
    return g_hE2ICC->Interpolate(dEg);
    break;
  default:
    cerr << "err: impossible transistion" << endl;
  }
...
///////////////// Calculate Widths /////////////////////////////////////////////
double GetWidth(int nExI, int nSpbI, int nParI, int nLvlInBinI, int nReal, double *adConWid, double *adDisWid,
                TRandom2 *arConState) {

  TRandom2 ranStr(1 + nReal +
                  g_nReal * (g_anConCumul[EJP(nExI, nSpbI, nParI)] - g_anConLvl[EJP(nExI, nSpbI, nParI)] +
                             nLvlInBinI)); // seed with lvl num
  ...
  if (nExI >= 0) { // in constructed scheme
    ...
    for (int lvl = 0; lvl < g_nDisLvlMax; lvl++) {
      ...
      if (g_bIsEvenA)
        nSpbF = int(dSpF + 0.001);
      else
        nSpbF = int(dSpF - 0.499);
    ...
    cerr << "err: Requesting discrete width" << endl;
  return dTotWid;
} // GetWidth
...
////////////////////////// Take Step ///////////////////////////////////////////
bool TakeStep(int &nConEx, int &nSpb, int &nPar, int &nDisEx, int &nLvlInBin, int &nTransMade, double &dMixDelta2,
              double dTotWid, int nReal, double *adConWid, double *adDisWid, TRandom2 *arConState, TRandom2 &ranEv) {
  ...
  if (nConEx >= 0) {
    ...
    if (dTotWid <= 0.0)
      return false; // dead state in constructed: no E2 options
    ...
    for (int lvl = 0; lvl < g_nDisLvlMax; lvl++) {
      ...
      if (adDisWid[lvl] > 1e-4)
        cerr << "err: discrete width uninit" << endl;
      if (dWidCumulative >= dRanWid) { // once adds to more than dRanWid, it decays
        if (!bFoundLvl) {              // for safety check
          ...
          nToDisEx = lvl;
          if (g_bIsEvenA)
            nToSpb = int(g_adDisSp[lvl] + 0.001);
          else
            nToSpb = int(g_adDisSp[lvl] - 0.499);
    ...
    for (int gam = 0; gam < g_anDisGam[nDisEx]; gam++) {
      ...
      if (dBRCumulative >= dRanBR) {              // once adds to more than dRanBR, it decays
        if (!bFoundLvl) {                         // for safety check
          ...
          if (g_bIsEvenA)
            nToSpb = int(g_adDisSp[nToDisEx] + 0.001);
          else
            nToSpb = int(g_adDisSp[nToDisEx] - 0.499);
  ...
  if (nToSpb < 0 || nToPar < 0 || nToLvlInBin < 0) { // error check
    cerr << endl
         << "err: JP: from " << endl
         << nSpb << (nPar ? "+" : "-") << ", Lvl: " << nDisEx << ", ConBin: " << nConEx << ";" << nLvlInBin << endl
         << "To " << endl
         << nToSpb << (nToPar ? "+" : "-") << ", Lvl: " << nToDisEx << ", ConBin: " << nToConEx << ";" << nToLvlInBin
         << endl
         << "Likely branching ratios from file don't add to 1.000000" << endl
         << "Check level " << nDisEx << " in " << "zFile" << " manually." << endl;
...
///// Beta-decay like selection of states /////
  ...
  while (!bFoundSpin) {
    ...
    while (!bFoundLvl && nAttempt < nMaxAttempt) { // dont pop not existent lvls
      ...
      if (nExI < 0)
        cerr << "err: ExI below constructed scheme" << endl;
    ...
    for (int binJ = 1; binJ <= g_nSpPopIBin; binJ++) {
      ...
      if (!bEJLvlSuggested && (dPopSum > dRanPop)) {
        ...
        if (dLowEBdy < g_dECrit + 0.001) { // is discrete
          ...
          if (!bDisBinFound)
            cerr << "err: no discrete match" << endl;
          if (g_bIsEvenA)
            nSpbI = int(g_adDisSp[nDisEx] + 0.001);
          else
            nSpbI = int(g_adDisSp[nDisEx] - 0.499);
  ...
  if (!bEJLvlSuggested)
    cerr << "err: lvl not suggested" << endl;
  if (!bLvlMatch)
    cerr << "err: lvl not found" << endl;
...
void RAINIER(int g_nRunNum = 1) {
  ...
  for (int real = 0; real < g_nReal; real++) {
    ...
    for (int exim = 0; exim < g_nExIMean; exim++) {
      ...
      GetExI(nExI1, nSpbI1, nParI1, nDisEx1, nLvlInBinI1, ranEv1, 1.0, 1.0); // unused
...
#pragma omp for
        ...
        for (int ev = 0; ev < g_nEvent; ev++) {
          ...
          if (!((nCount * nThreads) % g_nEvUpdate))
            fprintf(stderr, "    approx: %d / %d \r", nCount * nThreads, g_nEvent);
#else
          if (!(ev % g_nEvUpdate))
          ...
          while (nDisEx > 0 && bIsAlive) { // nDisEx > g.s. (i.e. excited)
            ...
            // Fill populations
            if (nPar == 1)
              g_ah2PopLvl[real][exim]->Fill(nSpb + 0.5, dExPre);
            else
              g_ah2PopLvl[real][exim]->Fill(-nSpb - 0.5, dExPre);
      ...
      TH2D *g_ah2Temp[] = {g_ah2PopLvl[real][exim], g_ah2FeedTime[real][exim], g_ah2ExEg[real][exim],
                           g_ah21Gen[real][exim], g_ah2PopI[real][exim]};
      int nh2Temp = 5;
...
#define SAVE_ARR(stream, variable, size)                                                                               \
  ...
  TTimeStamp tEnd;
  double dElapsedSec = double(tEnd.GetSec() - tBegin.GetSec()) + double(tEnd.GetNanoSec() - tBegin.GetNanoSec()) / 1e9;
